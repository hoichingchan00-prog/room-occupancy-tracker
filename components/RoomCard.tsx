...
export function RoomCard({
  ...
  return (
    <article
      className={`flex flex-col rounded-2xl border border-stone-200/80 bg-white p-5 shadow-[0_10px_30px_-18px_rgba(28,25,23,0.35)] transition-all ${
        isClosed ? "opacity-75 bg-stone-50/50" : ""
      }`}
    >
      {/* 頂部校舍名稱與右側標籤/按鈕 */}
      ...
      {/* 不開放警示橫條 */}
      {isClosed && (
        <div className="mt-3 p-2 rounded-xl bg-rose-50 border border-rose-200 text-rose-700 text-center text-xs font-semibold">
          ⚠️ 今日暫不開放
        </div>
      )}
      ...
      <button
        type="button"
        onClick={onUnlock}
        className="mt-6 w-full py-2 text-xs font-semibold rounded-xl border border-stone-200 hover:bg-stone-100 transition-colors text-stone-700"
      >
      ...
      <div className="mt-2 grid grid-cols-2 gap-2">
        <button
          type="button"
          disabled={!isStaff || isClosed || pending || atMin}
          onClick={() => onAdjust(-1)}
        ...
        <button
          type="button"
          disabled={!isStaff || isClosed || pending}
          onClick={() => onAdjust(1)}
